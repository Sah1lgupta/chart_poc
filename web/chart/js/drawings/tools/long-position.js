...
DrawingManager.registerTool('longPos', {
  ...
  draw(ctx, points, style, isSelected, hoveredPtIdx) {
    ...
    const stopPrice = entryPrice * 0.975;  // 2.5% stop loss
    ...
    ctx.fillText('Target: +5.00%', minX + 4, targetY + 12);
    ctx.fillText('Risk/Reward: 2.00', minX + 4, a.y - 4);
    ctx.fillText('Stop Loss: -2.50%', minX + 4, stopY - 4);
  ...
  isHit(points, clickX, clickY) {
    ...
    const stopPrice = entryPrice * 0.975;
    const targetY = mainSeries.priceToCoordinate(targetPrice);
    const stopY = mainSeries.priceToCoordinate(stopPrice);
    ...
    return clickX >= minX - 5 && clickX <= maxX + 5 && clickY >= targetY - 5 && clickY <= stopY + 5;
  }
});
