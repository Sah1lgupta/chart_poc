// long-position.js — Long Position drawing tool.

DrawingManager.registerTool('longPos', {
  minPoints: 2,
  ...
  draw(ctx, points, style, isSelected, hoveredPtIdx) {
    ...
    if (points.length < 3) {
      ...
      const defaultStopPrice = entryPrice * 0.975; // -2.5% stop loss
    ...
    ctx.fillRect(minX, targetY, w, a.y - targetY);
    ctx.strokeStyle = '#26a69a';
    ctx.lineWidth = 1;
    ctx.strokeRect(minX, targetY, w, a.y - targetY);

    // Draw Stop Box (Red)
    ctx.fillStyle = 'rgba(239, 83, 80, 0.15)';
    ctx.fillRect(minX, a.y, w, stopY - a.y);
    ctx.strokeStyle = '#ef5350';
    ctx.strokeRect(minX, a.y, w, stopY - a.y);
    ...
    const stopDiff = entryPrice - stopPrice;
    ...
    ctx.fillText(`Target: +${targetPct}% (${targetPrice.toFixed(2)})`, minX + 4, targetY + 12);
    ctx.fillText(`R:R Ratio: ${rrRatio}`, minX + 4, a.y - 4);
    ctx.fillText(`Stop Loss: -${stopPct}% (${stopPrice.toFixed(2)})`, minX + 4, stopY - 4);

    if (isSelected) {
      // Draw handles at points[0], points[1], points[2]
  ...
  isHit(points, clickX, clickY) {
    ...
    // Use points[2] stop price if available
    const stopPrice = points.length >= 3 ? points[2].price : points[0].price * 0.975;
    const targetPrice = points[1].price;
